                                             ...
                                             void *data)
{
    ...
    /* namespace name should fall back to unknown since payload is untrusted and tag regex fails */
    ret = mp_kv_cmp(res_data, res_size,
                    "$resource['labels']['namespace_name']", "unknown");
    TEST_CHECK(ret == FLB_TRUE);

    /* pod name should fall back to unknown */
    ret = mp_kv_cmp(res_data, res_size,
                    "$resource['labels']['pod_name']", "unknown");
    TEST_CHECK(ret == FLB_TRUE);

    /* container name should fall back to unknown */
    ret = mp_kv_cmp(res_data, res_size,
                    "$resource['labels']['container_name']", "unknown");
    TEST_CHECK(ret == FLB_TRUE);
...
void flb_test_resource_k8s_container_untrusted_tag_wins()
{
    ...
    /* Ingest data sample with different local_resource_id in payload */
